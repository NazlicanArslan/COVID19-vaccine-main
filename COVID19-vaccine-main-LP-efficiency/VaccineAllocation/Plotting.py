...
def plot_austin_vs_cook():
    ...
    for (austin_file, cook_file, outfile_name) in zip(
        austin_files, cook_files, outfile_names
    ):
        ...
        if "death" in outfile_name:
            ax.scatter(
                df_austin_hosp["date"],
                np.cumsum(df_austin_hosp["hospitalized"]),
                label="Austin",
            )
            ax.scatter(
                df_cook_hosp["date"],
                np.cumsum(df_cook_hosp["hospitalized"]),
                label="Cook",
            )
        else:
            ax.scatter(
                df_austin_hosp["date"], df_austin_hosp["hospitalized"], label="Austin"
            )
            plt.plot(
                df_austin_hosp["date"][austin_peaks],
                df_austin_hosp["hospitalized"][austin_peaks],
                "x",
                c="black",
            )
            ax.scatter(df_cook_hosp["date"], df_cook_hosp["hospitalized"], label="Cook")
            plt.plot(
                df_cook_hosp["date"][cook_peaks],
                df_cook_hosp["hospitalized"][cook_peaks],
                "x",
                c="black",
            )
        fig.autofmt_xdate()
        plt.legend(loc="best")
        plt.savefig(
            "./plots/" + "unshifted_unscaled_estimated_" + outfile_name + ".png"
        )
        plt.show()
        # # shifted
        # plt.clf()
        ...
        if "death" in outfile_name:
            ax.scatter(
                df_austin_hosp["date"] + np.timedelta64(time_delta, "D"),
                np.cumsum(df_austin_hosp["hospitalized"]) * scale_factor,
                label="Austin",
            )
            plt.plot(
                df_austin_hosp["date"][austin_peaks] + np.timedelta64(time_delta, "D"),
                np.cumsum(df_austin_hosp["hospitalized"])[austin_peaks] * scale_factor,
                "x",
                c="black",
            )
            ax.scatter(
            ...
            plt.plot(
                df_cook_hosp["date"][cook_peaks],
                np.cumsum(df_cook_hosp["hospitalized"])[cook_peaks],
                "x",
                c="black",
            )
            ...
            ax.scatter(
                df_austin_hosp["date"] + np.timedelta64(time_delta, "D"),
                df_austin_hosp["hospitalized"] * scale_factor,
                label="Austin",
            )
            plt.plot(
                df_austin_hosp["date"][austin_peaks] + np.timedelta64(time_delta, "D"),
                df_austin_hosp["hospitalized"][austin_peaks] * scale_factor,
                "x",
                c="black",
            )
            ax.scatter(df_cook_hosp["date"], df_cook_hosp["hospitalized"], label="Cook")
...
def plot_region_sum_vs_state():
    ...
    for (state_file, region_sum_file, outfile_name) in zip(
        state_files, region_sum_files, outfile_names
    ):
        ...
        ax.scatter(
            df_region_sum_hosp["date"],
            df_region_sum_hosp["hospitalized"],
            label="region_sum",
        )
